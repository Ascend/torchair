...
def eliminate_self_copy(gm: torch.fx.GraphModule) -> torch.fx.GraphModule:
    ...
    These are generated by decompose_auto_functionalized when a mutable arg
    was already reinplaced (not in _only_clone_these_tensors). The copy_ is
    semantically a no-op but survives DCE because copy_ has side effects.
    """
    graph = gm.graph
    nodes_to_erase = []
    for node in graph.nodes:
        if node.target is torch.ops.aten.copy_.default and len(node.args) >= 2:
            if node.args[0] is node.args[1]:
                num_users = len(node.users)
                if num_users == 0:
                    nodes_to_erase.append(node)
    for node in nodes_to_erase:
        logger.debug("Eliminating self-copy node: %s", node)
        node.replace_all_uses_with(node.args[0])
        graph.erase_node(node)
    if nodes_to_erase:
        logger.debug("Eliminated %d self-copy nodes", len(nodes_to_erase))
    return gm