...
from typing import List, Optional, Callable, Any, Deque, Dict, Set
...
def get_node_all_placeholder_inputs(node, excluded_kwargs=None, excluded_arg_indices=None):
    ...
    while processing_input_queue:
        ...
        if isinstance(cur_input, Node):
            ...
            processing_input_queue.extend(cur_input.values())
...
def construct_fx_node_shape(ori_shape: List, sym_inputs: dict, graph_id: int) -> List:
    ...
    for dim in ori_shape:
        ...
        elif str(dim.node.expr).isdigit():
            # for number symbol, such as: sym(5)
            empty_shape.append(int(dim.node.expr))
        elif isinstance(dim.node.expr, sympy.Symbol):
            # for single symbol, such as: s0
...
class AclGraph(object):
    ...
    def reconstruct_outputs(self, graph_key: str) -> List:
        ...
        if "clone_output" not in self.config.keys() or not self.config["clone_output"]:
