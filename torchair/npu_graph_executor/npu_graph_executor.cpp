  ...
  for (size_t i = 0U; i < inputs.size(); ++i) {
    if (graph_data_->input_placements[i] == Placement::DEVICE) {
      if (!inputs[i].device().is_privateuseone()) {
        return Status::Error("Input %zu placement %s is incompatible with expected PrivateUse1.",
                             i, DebugString(inputs[i].device()).c_str());
      }
      if (is_first_run) {
      ...
      if (!inputs[i].device().is_cpu()) {
        return Status::Error("Input %zu placement %s is incompatible with expected CPU.",
                             i, DebugString(inputs[i].device()).c_str());
      }
      ...
      TNG_LOG(INFO) << "Assemble aten host input " << i << " "
                    << DebugString(retain_tmp_device_inputs.back())
...
Status StaticNpuGraphExecutor::Run(const std::vector <at::Tensor> &torch_inputs,
  ...
  TNG_LOG(INFO) << "StaticNpuGraphExecutor::Run graph " << graph_data_->id
                << " on stream " << stream << " successfully.";
  return Status::Success();
}
