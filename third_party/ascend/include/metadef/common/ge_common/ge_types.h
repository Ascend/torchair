...
#ifndef INC_COMMON_GE_TYPES_H_
#define INC_COMMON_GE_TYPES_H_
...
struct Command {
  ...
  uint32_t cache_flag;                  // clear prof cache flag
};
...
struct ModelData {
  ...
  std::string weight_path;     // weight path, used for load weight
};
...
struct ModelParam {
  ...
  size_t weight_size;
  uintptr_t fixed_mem_base;
  size_t fixed_mem_size;
};
