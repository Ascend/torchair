...
| 参数名 | 说明 |
| --- | --- |
| aclgraph.disable_reinplace_inplaceable_ops_pass | 布尔类型，是否关闭Pass。该Pass针对模型的中间节点，将节点中包含的[Out-of-place算子](简介.md#section643113494714)（非原地算子）替换为[In-place算子](简介.md#section643113494714)（原地算子），以减少计算过程中的内存搬运，从而提升性能。<br> - False（默认值）：默认打开此Pass。<br> - True：关闭此Pass。<br> **说明**： 不支持多原地算子（修改了多个输入的算子）开启本pass。 |
