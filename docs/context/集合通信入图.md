...
-   **PyTorch 2.1版本**：通过Monkey Patch方式接入Ascend IR计算图，解决原生集合通信算子不支持入图的问题。入图方法参见[使用方法（PyTorch 2.1版本）](#使用方法（PyTorch 2.1版本)。
-   **PyTorch 2.6及以上版本**：TorchAir提供的NPU图编译后端实现了集合通信算子的Ascend Converter，调用torch.compile时，默认已支持集合通信算子入图。入图方法参见[使用方法（≥PyTorch 2.6版本）](#使用方法（≥PyTorch 2.6版本）)。
...
上述接口详细介绍请参考《Ascend Extension for PyTorch 自定义API参考》。
## 使用方法（PyTorch 2.1版本）
