...
该功能通过[torchair.get\_npu\_backend](get_npu_backend.md)中compiler\_config配置，示例如下，仅供参考不支持直接拷贝运行，参数介绍参见[表1](#table283871313126)。
...
<tbody>
  ...
  <tr>
    ...
    <td>设置dump文件生成的路径，字符串型。可选配置，如果不设置，默认为当前执行路径。<br>请确保该参数指定的路径确实存在，并且运行用户具有读、写操作权限。
    </td>
  </tr>
</tbody>
</table>
...
| 参数名 | 参数说明 |
| --- | --- |
| data_dump.type | 指定dump文件类型，字符串类型。<br>  - 默认为None，表示不导出dump数据。<br>  - 若设置，当前仅支持npy格式。 |
| data_dump.filter | 用户自定义过滤函数，保留满足函数条件的内容。<br>输入：PyTorch中的[Node类](https://github.com/pytorch/pytorch/blob/bf5aa9e42eb4049aad56264dacefd638233924b5/torch/fx/node.py)的实例n。<br>输出：PyTorch中的[Node类](https://github.com/pytorch/pytorch/blob/bf5aa9e42eb4049aad56264dacefd638233924b5/torch/fx/node.py)的实例n或者None。<br>默认值为None，表示不过滤任何内容。<br>说明：Node类实例化的各属性（如name、target等）获取方法主要通过[图结构dump功能](图结构dump功能.md)获取。<br>以常见的name属性为例，获取方法如下：<br><br>  1. 先以py格式dump图信息。config.debug.graph_dump.type = "py"<br>  2. 在当前执行路径下生成dynamo_*.py，示例如下，搜索关键词“FX Code”，其后面字段对应n.name属性信息。# File "/home/a.py", line 32, in forward  x=x+y<br>## FX Code: add_1: torch.float32[s0, s0]npu:0 = torch.ops.aten.add.Tensor(add: torch.float32[s0, s0]npu:0, arg2_1: torch.float32[s0, s0]npu:0)<br>Add_1_0 = ge.Add(Add_0, arg2_1_0, node_name="Add_1")<br><br># File "/home/a.py", line 36, in forward  x=x-1<br>## FX Code: sub: torch.float32[s0, s0]npu:0 = torch.ops.aten.sub.Tensor(mul_1: torch.float32[s0, s0]npu:0, 1)<br>Sub_0 = ge.Sub(Cast_1_0, ge.Const(1, dtype=0), node_name="Sub") |
| data_dump.path | 设置dump文件生成的路径，字符串型。可选配置，如果不设置，默认为当前执行路径。<br>请确保该参数指定的路径确实存在，并且运行用户具有读、写操作权限。 |
