...
用户可通过自定义图优化Pass，将其注册到npugraph_ex中，达到修改PyTorch FX图的目的。

## 使用约束

npugraph_ex约定FX Pass的函数签名如下：

```python
def _(gm, example_inputs, config: npugraph_ex.configs.conpiler_confg.CompilerConfig) -> None
```
...
- config：表示npugraph_ex编译后端创建的[CompilerConfig类](../api/npugraph_ex/compiler_config.md)对象，用于Pass感知完整编译选项。
...
2. 将自定义Pass注册到npugraph_ex使其生效。
    ...
    当前npugraph_ex的编译选项通过torchair.CompilerConfig类传递，需要开启post_grad_custom_pre_pass和post_grad_custom_post_pass两个阶段的自定义Pass注册。开关开启方式如下：
    ...
    **表 1**  参数说明
    ...
    |参数名| 说明                                                                 |
    |--|--------------------------------------------------------------------|
    |post_grad_custom_pre_pass| npugraph_ex本身内置了部分FX图优化Pass，该配置控制自定义Pass在内置Pass执行前生效。传入自定义Pass函数。 |
    |post_grad_custom_post_pass| npugraph_ex本身内置了部分FX图优化Pass，该配置控制自定义Pass在内置Pass执行后生效。传入自定义Pass函数。 |

3. 检查Pass是否生效。
    - 打开Debug日志（export TORCH_COMPILE_DEBUG=1），查看修改后的FX图是否有插入torch.ops.air.scope_enter.default和torch.ops.air.scope_exit.default、torch.ops.air.record.default、torch.ops.air.wait.default新节点，以及插入的位置是否正确。
