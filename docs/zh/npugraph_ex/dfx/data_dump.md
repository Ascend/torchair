...
|data_dump_stage|dump数据阶段，字符串类型。<br>original：dump最原始的、未经过npugraph\_ex优化的fx图。注意，开启该模式后，会通过eager mode只执行最原始的fx图，不会进入aclgraph模式。<br>optimized（默认值）：dump经过npugraph\_ex优化后的fx图。|
