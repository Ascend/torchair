...
**图 1**  max-autotune执行时间分布示意图  
![](figures/max-autotune执行时间分布示意图.png "max-autotune执行时间分布示意图")
...
-   max-autotune模式下，本功能跳过了Dynamo的JIT编译环节、Guards、Ascend IR图编译环节，与torch.compile原始方案相比多了如下限制：
