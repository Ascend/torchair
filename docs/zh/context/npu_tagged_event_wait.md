...
注意wait与record需匹配使用，在Device上执行时，wait及之后的算子需要等待与之匹配的record执行完成后才能执行。详细功能介绍参见[图内多流表达功能（aclgraph）](图内多流表达功能（aclgraph）.md)。

## 函数原型

```python
npu_tagged_event_wait(event)
```
