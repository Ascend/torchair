...
-   **前提条件**：在使用TorchAir图模式功能之前，建议先熟悉Ascend Extension for PyTorch基础知识。
    ...
    -   参考[《PyTorch 训练模型迁移调优指南》](https://www.hiascend.com/document/redirect/canncommercial-ptmigr)的“模型脚本迁移”章节，了解如何将模型迁移至昇腾NPU上等。
-   **产品支持情况：**
    通常情况下，功能默认支持所有产品。如有特殊情况，将在相应章节的“使用约束”中说明。
    其中reduce-overhead模式（aclgraph）功能目前仅支持如下产品：
