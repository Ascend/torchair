...
-   **场景2：**
    ...
    由于aclgraph基于固定内存地址执行，因此前次执行的输出Tensor内存可能会被后续执行覆盖。如需将输出结果长时间保存和使用，可能会引入精度问题。针对该问题，可以在模型脚本中使用output_clone=output.clone()形式对指定输出结果做克隆，并使用clone后的Tensor参与后续执行。
