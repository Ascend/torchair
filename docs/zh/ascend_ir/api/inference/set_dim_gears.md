...
开启[动态shape图分档执行功能](../../features/advanced/dynamic_gears_merge_policy.md)时需要调用该接口设置图被划分的档位。
...
|--|--|--|
|t|输入|待分档的输入Tensor。|
|dim_gears|输入|用于设置Tensor不同dim维度下的档位值。输入类型为Dict（{key:value}键值对形式），其中key为dim维度（整型），value为档位值（整数列表或元组）。|
