...
推理场景下，Ascend Extension for PyTorch提供的**torch\_npu.npu\_fused\_infer\_attention\_score**（参考《Ascend Extension for PyTorch 自定义 API参考》中的“torch\_npu.npu\_fused\_infer\_attention\_score”章节），适配增量和全量推理场景的FlashAttention算子，既可以支持全量计算场景（PromptFlashAttention），也可支持增量计算场景（IncreFlashAttention）。
