...
|参数|输入/输出|说明|
|--|--|--|
|其他参数|输入|与**torch_npu.npu_fused_infer_attention_score_v2接口**同名参数要求一致。|
|actual_seq_qlen|输入|Tensor类型，代表不同Batch中query的有效Sequence Length，数据类型支持int64。|
|actual_seq_kvlen|输入|Tensor类型，代表不同Batch中key/value的有效Sequence Length，数据类型支持int64。|
...
class Model(torch.nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self):
        return tng.ops.npu_fused_infer_attention_score_v2(q, k, v, actual_seq_qlen = actualSeqLengths, actual_seq_kvlen = actualSeqLengthkvs, num_query_heads = 8, input_layout = "BNSD", scale=scale, pre_tokens=65535, next_tokens=65535)
def MetaInfershape():
    with torch.no_grad():
        model = Model()
