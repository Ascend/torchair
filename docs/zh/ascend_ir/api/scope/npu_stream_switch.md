...
|enable_inner_parallel|输入| bool类型，表示是否使能stream_tag流内的算子按照GE原有的并发策略分流，默认开启。<br>**说明**： <br>- GE图中的算子将自动添加_enable_inner_parallel属性，属性值与enable_inner_parallel参数值一致。<br>- _enable_inner_parallel属性的详细介绍和约束请参见[CANN GE图引擎 API](https://hiascend.com/document/redirect/CannCommunityAscendGraphApi)中的“属性名列表”章节，并且配套CANN版本大于9.0.0时才生效。 |
