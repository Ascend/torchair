...
- 调用torch\_npu.npu\_prefetch接口时（参考[Ascend Extension for PyTorch文档中心](https://hiascend.com/document/redirect/pytorchuserguide)中的《Ascend Extension for PyTorch 自定义 API参考》“torch\_npu.npu\_prefetch”章节），不支持同时开启本功能，否则影响算子正常执行。
