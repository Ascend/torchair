...
- UB融合（Unified Buffer Fusion）：指对图上使用UB的算子进行融合优化。例如两个算子单独运行时，算子1的计算结果存储在UB上，需搬移到DDR（Double Data Rate，双倍速率同步动态随机存储器）。算子2执行时，需要将算子1的输出从DDR搬移到UB，再进行算子2的计算，完成后再从UB搬移回DDR。
    ...
    从上述过程可知，算子1的结果经历了从UB-\>DDR-\>UB-\>DDR，过程中的DDR数据搬移是不必要的。可以将算子1和算子2合并成一个算子，融合后算子1的数据直接保留在UB中，算子2直接从UB获取数据进行算子2的计算，这样就节省了一次输出DDR和一次输入DDR，减少了数据搬移时间，提高运算效率，有效降低了带宽。
...
2. 使能算子融合规则配置文件。
    ...
    **表 1**  参数说明
    ...
    若想确认该功能是否生效，在运行前需提前设置日志环境变量（参见[TorchAir C++层日志打印](../basic/cplus_log_print.md)），其会打印图初始化时的一些开关配置。
