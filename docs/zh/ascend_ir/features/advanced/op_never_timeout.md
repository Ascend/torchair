...
2.  指定算子配置不超时。
    ...
    使用如下with语句块（[op\_never\_timeout](../../api/scope/op_never_timeout.md)），enable为bool类型，置为True时语句块内算子将自动添加 \_op\_exec\_never\_timeout属性，不会参与超时检测。

    > [!NOTE]说明
    >关于 \_op\_exec\_never\_timeout属性的详细介绍和约束请参见《CANN GE图引擎接口》中“数据类型\>属性名列表”章节。
...
设置成功后，参考[TorchAir Python层日志打印](../basic/python_log_print.md)开启Debug日志，可以看到类似的提示信息：
