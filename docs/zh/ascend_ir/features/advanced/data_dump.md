...
本功能可以Dump Ascend IR图执行时的算子输入、输出数据，用于后续问题定位和分析，如算子运行性能或精度问题。
...
对于[表1](#fig1)提供的dump options，TorchAir提供了更灵活的dump算子范围。通过[data\_dump](../../api/scope/data_dump.md)接口实现，支持与表中其他所有dump options配套使用。
...
1.  通过DUMP\_GE\_GRAPH环境变量dump整个流程各阶段的图描述信息，建议取3（精简版dump，即只打印节点关系），详细介绍参见《CANN 环境变量参考》中的“DUMP\_GE\_GRAPH”章节。
...
-   **使用说明**：
    ...
    -   注意torch\_npu默认已开启exception类信息的dump功能（即dump\_scene参数，异常算子Dump配置） ，**通过本功能配置的exception dump不会生效**。
    -   对于大模型场景，通常dump数据量太大并且耗时长，建议dump\_data配置为“stats”，开启算子统计功能，根据统计数据识别可能异常的算子后，再dump可能异常的算子。
