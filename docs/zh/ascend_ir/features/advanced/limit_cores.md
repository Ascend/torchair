...
更多关于AI Core和Vector Core的介绍请参考[AI Core/Cube Core/Vector Core简介](../../../appendix/appendix/aicore.md)，如需了解Eager和图模式下的控核差异请参考[Eager和图模式下控核介绍](../../../appendix/appendix/core_limit.md)。
...
2.  （可选）配置算子级核数。
    ...
    使用如下with语句块（[limit\_core\_num](../../api/scope/limit_core_num.md)），语句块内的算子均按照入参指定核数。
    ...
    参考[图结构dump功能](../basic/graph_dump.md)获取完整的图结构信息，如config.debug.graph\_dump.type="txt"，再查看生效的算子级核数，一般在“attr”属性中，key名分别为“\_op\_aicore\_num”和“\_op\_vectorcore\_num”。

3.  （可选）配置全局核数。

    该功能通过[torchair.get\_npu\_backend](../../api/torchair/get_npu_backend.md)中compiler\_config配置来指定全局核数，示例如下，参数说明参见下表。
