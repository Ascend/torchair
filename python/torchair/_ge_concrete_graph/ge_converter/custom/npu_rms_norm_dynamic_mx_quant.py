...
@declare_supported([
    Support(F16(4, 3), F16(3)),
    Support(F16(4, 3), F16(3), beta=F16(3))
])
@register_fx_node_ge_converter(torch.ops.npu.npu_rms_norm_dynamic_mx_quant.default)
def conveter_npu_rms_norm_dynamic_mx_quant_default(
    x: Tensor,
    gamma: Tensor,
    ...
    """
    NB: aten::npu_rms_norm_dynamic_mx_quant(Tensor x, Tensor gamma, *, Tensor? beta=None, 
                                            float epsilon=1e-06, int scale_alg=0, str round_mode="rint", 
                                            int dst_type=296) -> (Tensor, Tensor, Tensor)
    """
    # 输出dst_type类型Pytorch不支持，转换
    acl_dst_type = torch_dtype_value_to_ge_type(dst_type)
    y, mxscale, rstd = ge.RmsNormDynamicMxQuant(x, gamma, beta=beta, epsilon=epsilon, scale_alg=scale_alg, round_mode=round_mode, dst_type=acl_dst_type, output_rstd=False)
    ...
    if dst_type == 296 or dst_type == 297:
        ...
        return ge.Reshape(y, y_shape_uint8), mxscale, rstd
    else:
