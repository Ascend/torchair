...
def guard_view_input(func):
    def wrapper(self, target: 'Target', args: Tuple[Argument, ...], kwargs: Dict[str, Any], meta_outputs: Any):
        if self.config.experimental_config.enable_view_optimize:
            ...
            stream_label = current_attributes.get('_user_stream_label', None)
            scope = current_attributes.get('_super_kernel_scope', None)
            if is_view_case(target, args, meta_outputs) and (stream_label is None) and (scope is None):
                logger.debug(f"Do view optimize, skip the operator {str(target)}, "
                             f"stream_label: {str(stream_label)}, scope: {str(scope)}")
