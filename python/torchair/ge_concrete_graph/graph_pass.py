...
def _find_packs_and_io_ops(graph: GraphDef):
    ...
    for op in graph.op:
        if op.type == "Data" and op.input_desc[0].device_type == "CPU":
            ...
            if not op.attr["axis"].i == 0: raise AssertionError(f"sym pack op attr axis must be 0.")
...
def optimize_sym_pack(graph: GraphDef, inputs: List, placements: List, fx_inputs_mapping: Dict, fx_inputs_len: int):
    ...
    if not len(placements) == len(
        inputs): raise AssertionError(f"graph inputs size {len(inputs)} is not equal to input_placements size {len(placements)}")
    if not len(fx_inputs_mapping) == len(
        inputs): raise AssertionError(f"graph inputs size {len(inputs)} is not equal to input_mappings size {len(fx_inputs_mapping)}")
