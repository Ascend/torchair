...
def conveter_aten_native_batch_norm_backward_default(
    ...
    if not train:
        ...
        raise RuntimeError(
            "torch.ops.aten.native_batch_norm_backward.default ge_converter is not implemented while train is False!"
        )
    if save_mean is None or save_invstd is None:
        raise RuntimeError("torch.ops.aten.native_batch_norm_backward.default "
                           "ge_converter is not implemented while save_mean is None or save_invstd is None!")
    ...
    if dim <= 4:
        ...
        grad_in = ge.BNTrainingReduceGrad(grad_out, input, diff_scale, diff_offset, 
                                          weight, save_mean, save_invstd, epsilon=eps)
