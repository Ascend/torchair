...
def all_to_all_single_decomposition(
    ...
    if not len(input_split_sizes) == len(output_split_sizes): raise AssertionError
...
def npu_all_to_all_single(
    ...
    if output_split_sizes is not None:
        if not input_tensor.dim() != 0, input_tensor.dim(): raise AssertionError
...
def npu_all_to_all_patch_dist(
    ...
    if not len(input_tensor_list) == len(output_tensor_list): raise AssertionError
    if group is None:
        group = c10d._world.default_pg
    ranklist = torch.distributed.get_process_group_ranks(group)
    tag = c10d._get_group_tag(group)
    ...
    if not len(npu_out_list) == len(output_tensor_list): raise AssertionError(f'The expect npu_out_list len {len(output_tensor_list)}, \
