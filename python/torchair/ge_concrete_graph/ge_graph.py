...
def auto_convert_to_tensor(inputs_dynamic, inputs_optional, *, inputs_tensor_type=None):
    def inner(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not len(inputs_dynamic) == len(inputs_optional): raise AssertionError
            if not len(args) >= len(inputs_dynamic): raise AssertionError
            ...
            for i, dynamic_and_optional in enumerate(zip(inputs_dynamic, inputs_optional)):
                ...
                arg = args[i]
                if dynamic:
                    if optional: raise AssertionError("Optional input cannot be dynamic")
                    if not isinstance(arg, (list, tuple)): raise AssertionError
                    if not all([isinstance(v, Tensor) for v in arg]): raise AssertionError
                else:
                    if arg is None:
                        if not optional: raise AssertionError(f"Input {i} can not be None as it is not optional")
                    else:
                        if not isinstance(arg, Tensor): raise AssertionError
...
def compat_as_bytes_list(bytes_or_text, encoding='utf-8'):
    if not isinstance(bytes_or_text, (list, tuple)): raise AssertionError
    return [compat_as_bytes(v) for v in bytes_or_text]


def trans_to_list_list_int(lli):
    if not isinstance(lli, (list, tuple)): raise AssertionError
    attr = AttrDef.ListListInt()
    for li in lli:
        if not isinstance(li, (list, tuple)): raise AssertionError
...
def trans_to_list_list_float(llf):
    if not isinstance(llf, (list, tuple)): raise AssertionError
    attr = AttrDef.ListListFloat()
    for lf in llf:
        if not isinstance(lf, (list, tuple)): raise AssertionError
...
def Data(*, index: int, dtype: int, shape: List[int] = None, format: str = "ND", placement: str,
    ...
    desc.layout = format
    if not placement in ["NPU", "CPU"]: raise AssertionError(f"placement should be NPU or CPU, but got {placement}")
