...
def install_or_compile_static_kernel(fx_func, *args, cached_cann_version=None, compile_cache_dir=None, cached_deterministic=None, **kwargs):
    ...
    if _is_single_card():
        _install_or_compile_for_single(fx_func, *args, cached_cann_version=cached_cann_version,
                                       ...
                                       cached_deterministic=cached_deterministic, **kwargs)
    else:
...
def _install_or_compile_for_single(fx_func, *args, cached_cann_version=None, compile_cache_dir=None, cached_deterministic=None, **kwargs):
...
def _install_or_compile_for_multi(fx_func, *args, cached_cann_version=None, compile_cache_dir=None, cached_deterministic=None, **kwargs):
    # 多卡流程
...
def _compile_static_kernel_for_single_card(fx_func, *args, is_cache_compile=False, build_dir=None, **kwargs):
    ...
    # 2.开始静态编译
    compile_result = static_compile(result_root, chosen_dir)
...
def _compile_static_kernel_for_multi_card(fx_func, *args, gloo_group=None, is_cache_compile=False, build_dir=None, **kwargs):
    rank = dist.get_rank()
    result_root = safe_resolve_output_dir(build_dir)
    warnings.warn(
...
def _merge_and_compile_install(gathered_json_dirs: list[Path], result_root: Path, rank):
    ...
    # 静态编译
    compile_result = static_compile(result_root, gathered_opcompile_dir, rank)
    if not compile_result:
        return
...
def static_compile(result_root: Path, chosen_dir, rank: int = None) -> bool:
    # 开始静态编译
    prefix = _get_log_prefix(rank)
    try:
        cmd = [
