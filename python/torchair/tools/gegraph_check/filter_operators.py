...
激活函数等）。死锁检测只关心两类 task：
  ...
  2. 通信类（Communication）：占用 AIVEC/AIC 硬件资源，Schem Mode=1 时可能引发死锁
...
通信类算子若 Schem Mode ≠ 1，会打印告警。
Schem Mode=1 是触发死锁的必要条件（需凑满 Numblocks 个核才能启动），
Schem Mode≠1 的通信算子无需纳入死锁检测，通常不应出现，值得关注。
...
def _filter_operators(input_path: str, output_path: str, merge_gap: float = 0.0):
    ...
    for task in tasks:
        ...
        if op_type is not None:
            ...
            # 通信算子 Schem Mode 不为 1 时告警
            if op_type == "communication":
                schem_mode = task.get("args", {}).get("Schem Mode")
                if schem_mode != 1:
                    task_id = task.get("args", {}).get("Task Id", "?")
                    print(f"  [告警] 通信算子 Schem Mode≠1: {stream} Task={task_id} "
                          f"name=\"{name}\" Schem Mode={schem_mode}")
        else:
