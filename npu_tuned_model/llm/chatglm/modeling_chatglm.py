...
class CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        if query_layer.size(2) > 1:
            ...
            context_layer = torch_npu.npu_incre_flash_attention(query_layer.contiguous(),
                                                                 key_layer.contiguous(),
                                                                 value_layer.contiguous(),
                                                                 num_heads=query_layer.size(1),
                                                                 input_layout="BNSD",
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        if exe_mode == "dynamo" and not compiled:
            ...
            self._wrap_graph = torch.compile(self._wrap_graph, dynamic=True, fullgraph=True, backend=npu_backend)
            compiled = True
        
        hidden_states, presents, all_hidden_states, all_self_attentions = self._wrap_graph(position_ids,
                                                                                           seq_length,
                                                                                           inputs_embeds,
                                                                                           full_attention_mask,
                                                                                           past_key_values,
