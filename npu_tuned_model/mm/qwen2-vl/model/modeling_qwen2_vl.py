...
def apply_rotary_pos_emb_vision(tensor: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    ...
    cos = cos.unsqueeze(1).repeat(1, 1, 2).unsqueeze(0).float()
    sin = sin.unsqueeze(1).repeat(1, 1, 2).unsqueeze(0).float()
    #output = torch_npu.npu_rotary_mul(tensor, cos, sin)
    output = (tensor * cos) + (rotate_half(tensor) * sin)
    output = output.to(orig_dtype)
    return output
...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        rotary_pos_emb: torch.Tensor = None, attention_mask: torch.Tensor = None
    ) -> torch.Tensor:
        seq_length = hidden_states.shape[0]
        q, k, v = self.qkv(hidden_states).reshape(seq_length, 3, self.num_heads, -1).permute(1, 0, 2, 3).unbind(0)
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, rotary_pos_emb, attention_mask) -> torch.Tensor:
        hidden_states = hidden_states + self.attn(
...
class Qwen2VLAttention(nn.Module):
    ...
    def _flash_attn(self, q_len, query_states, key_states, value_states, attention_mask=None):
        attention_mask = attention_mask.to(torch.bool)
        if q_len > 1:
    ...
    def forward(
        ...
        attn_output = self._flash_attn(q_len, query_states, key_states, value_states, attention_mask)
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def _forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, rotary_pos_emb: torch.Tensor,
                cu_seqlens: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        hidden_states = self.patch_embed(hidden_states)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    def __init__(self, config: Qwen2VLConfig):
        ...
        self.max_position_embeddings = config.max_position_embeddings
        self.rope_theta = config.rope_theta
        self.mrope_section = self.config.rope_scaling["mrope_section"] * 2
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            and attention_mask.device.type == "npu"
            and not output_attentions
        ):
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.norm = LayerNorm(self.config.vision_config.embed_dim, eps=1e-6)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.get_dtype())
                embed_hidden_states = self.patch_embed(pixel_values)
                embed_hidden_states = self.norm(embed_hidden_states)
                ...
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw, rotary_pos_emb=rotary_pos_emb,
            ...
            if pixel_values_videos is not None:
                pixel_values_videos = pixel_values_videos.type(self.visual.get_dtype())
                embed_hidden_states = self.patch_embed(pixel_values_videos)
                embed_hidden_states = self.norm(embed_hidden_states)
                seq_length = embed_hidden_states.shape[0]
                embed_attention_mask = torch.full(
                    [1, seq_length, seq_length],
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None and prompt_length != cache_position.shape[0]:
