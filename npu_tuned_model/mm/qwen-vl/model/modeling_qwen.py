...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.logn_tensor = torch.tensor(logn_list, dtype=config.torch_dtype, device=self.device)[None, :, None, None]
    ...
    def _flash_attn(self, q_len, query, key, value, kv_padding_size, actual_seq_len, attention_mask=None):
        attention_mask = attention_mask.to(torch.bool)
        if q_len > 1:
            attn_output = torch_npu.npu_prompt_flash_attention(query, key.contiguous(),
    ...
    def forward(
        ...
        attn_output = self._flash_attn(q_len, query, key, value, kv_padding_size, actual_seq_len, attention_mask)
        context_layer = self._merge_heads(
            attn_output, self.num_heads, self.head_dim
        )
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if images is not None:
            torch_npu.scatter_update_(hidden_states, img_pos[:, 1] + 1, images, -2)
        output_shape = input_shape + (hidden_states.size(-1),)
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.image_transform = transforms.Compose([
            transforms.Resize(
                (config.visual['image_size'], config.visual['image_size']),
    ...
    def compute_rotary_pos_emb(self, input_ids, past_key_values, prompt_length):
        # past key values[0][0] shape: bs * seq_len * head_num * dim
        ...
        if (
            ...
            ntk_alpha = self.rotary_emb._ntk_alpha_cached
        rotary_pos_emb = self.rotary_emb(self.config.max_position_embeddings, ntk_alpha=ntk_alpha)
        if past_key_values is None:
            rotary_pos_emb = [i[:, 0:prompt_length, :, :] for i in rotary_pos_emb]
        else:
            rotary_pos_emb = [i[:, prompt_length - 1:prompt_length, :, :] for i in rotary_pos_emb]
    ...
    def prepare_inputs_for_generation(
        ...
        rotary_pos_emb = self.compute_rotary_pos_emb(input_ids, past_key_values, prompt_length)

        images, img_pos = self.preprocess_images(input_ids, past_key_values)

        past_key_values, attention_mask, position_ids = self.prepare_kv_cache(input_ids, past_key_values,
        ...
        model_inputs.update(
            {
                ...
                "images": images,                        # 新增
                "img_pos": img_pos,                      # 新增
                "rotary_pos_emb": rotary_pos_emb,        # 新增
                "kv_padding_size": kv_padding_size,      # 新增
                "actual_seq_len": actual_seq_len,        # 新增
            }
        )
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states, residual: Optional[torch.Tensor] = None):
        if residual is None:
            return torch_npu.npu_rms_norm(hidden_states, self.weight, self.eps)[0], hidden_states
        else:
            y, _, x = torch_npu.npu_add_rms_norm(residual, hidden_states, self.weight, self.eps)
        return y, x
